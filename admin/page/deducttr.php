<?php
class page_deducttr extends Page {
	function init() {
		parent::init();
		$this->api->auth->check();

		//$tab = $this->add('Tabs');
		//$obj = $tab->addTab('Other Deductions')->add('MVCGrid')->setModel('DeductTrans');
		$obj = $this->add('MVCGrid')->setModel('DeductTrans');
		...
		if($_GET['defer']){
			if($obj->owner){
				list($pp,$opp) = $this->api->sql->queryOneTableList('payrollperiod,origpayrollperiod','deducttrans','id=%i',$_GET['defer']);
				...
				$this->api->sql->update('deducttrans',$f,'id=%i',$_GET['defer']);
		...
		if($_GET['reset']){
			if($obj->owner){
				list($pp,$opp) = $this->api->sql->queryOneTableList('payrollperiod,origpayrollperiod','deducttrans','id=%i',$_GET['reset']);

				if(!empty($opp)){
					$f = array();
					$f['payrollperiod'] = $opp;
					$f['origpayrollperiod'] = null;
					$this->api->sql->update('deducttrans',$f,'id=%i',$_GET['reset']);
				}
