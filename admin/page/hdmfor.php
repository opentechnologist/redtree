<?php
class page_hdmfor extends Page {
	function init() {
		parent::init();
		$this->api->auth->check();

		$obj = $this->add('MVCGrid')->setModel('HDMFOverRide');
		if($obj->owner){
			...
			if($obj->owner)
				$obj->owner->js()->univ()
					->frameURL('Deduction Override',
						$this->api->getDestinationURL('hdmfae',
		...
		if($_GET['reset']){
			...
			$this->api->sql->updateOneTableField('hdmftable_id',0,'overrides',
