<?php
class page_phor extends Page {
	function init() {
		parent::init();
		$this->api->auth->check();

		$obj = $this->add('MVCGrid')->setModel('PHOverRide');
		if($obj->owner){
			...
			if($obj->owner)
				$obj->owner->js()->univ()
					->frameURL('Deduction Override',
						$this->api->getDestinationURL('phae',
		...
		if($_GET['reset']){
			...
			$this->api->sql->updateOneTableField('phtable_id',0,'overrides',
