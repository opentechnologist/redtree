<?php
class page_sssor extends Page {
	function init() {
		parent::init();
		$this->api->auth->check();

		$obj = $this->add('MVCGrid')->setModel('SSSOverRide');
		if($obj->owner){
			...
			if($obj->owner)
				$obj->owner->js()->univ()
					->frameURL('Deduction Override',
						$this->api->getDestinationURL('sssae',
		...
		if($_GET['reset']){
			...
			$this->api->sql->updateOneTableField('ssstable_id',0,'overrides',
