<?php
class page_incometr extends Page {
	function init() {
		parent::init();
		$this->api->auth->check();

		//$tab = $this->add('Tabs');
		//$obj = $tab->addTab('Other Income')->add('MVCGrid')->setModel('IncomeTrans');
		$obj = $this->add('MVCGrid')->setModel('IncomeTrans');
		...
		if($_GET['defer']){
			if($obj->owner){
				list($pp,$opp) = $this->api->sql->queryOneTableList('payrollperiod,origpayrollperiod','incometrans','id=%i',$_GET['defer']);
				...
				$this->api->sql->update('incometrans',$f,'id=%i',$_GET['defer']);
		...
		if($_GET['reset']){
			if($obj->owner){
				list($pp,$opp) = $this->api->sql->queryOneTableList('payrollperiod,origpayrollperiod','incometrans','id=%i',$_GET['reset']);

				if(!empty($opp)){
					$f = array();
					$f['payrollperiod'] = $opp;
					$f['origpayrollperiod'] = null;
					$this->api->sql->update('incometrans',$f,'id=%i',$_GET['reset']);
				}
